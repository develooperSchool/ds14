...
const err = require("../../errors/InvalidTimeTableError");

const postTimeValidation = (req, res, next) => {
  const { sub_id, time, user_id } = req.body;

  if (utils.IsInvalidNameNum(time)) {
    throw new err.InvalidTime("ENTER CORRECT TIME FIELD", res);
  }
...
const putTimeValidation = (req, res, next) => {
  ...
  if (utils.isInvalidId(req.params.id)) {
    throw new err.InvalidId("ENTER CORRECT ID", res);
  }
  if (utils.isInvalidId(sub_id)) {
    throw new err.InvalidSubId("ENTER CORRECT ID", res);
  }
